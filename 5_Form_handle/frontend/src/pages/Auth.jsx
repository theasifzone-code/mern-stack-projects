    ...
    return (
        <div className='flex items-center justify-around h-screen '>
            ...
            <div className=' bg-gradient-to-r from-purple-500 via-slate-300 to-red-500 rounded-2xl p-10  flex flex-col justify-center items-center gap-3 w-1/3  '>
                <h1 className='text-3xl font-bold mb-3'>{isSignUp ? "SignUp" : "Login"}</h1>
                {/* form  */}
                ...
                <div className='flex items-center gap-1'>
                    <p>{isSignUp ? "Already have an account? ":"please first create account. "} </p><button onClick={handleNavigate} className='underline font-bold hover:cursor-pointer'>{isSignUp ? "Login" : "SignUp"}</button>
                </div>
            </div>
        </div>
    )
